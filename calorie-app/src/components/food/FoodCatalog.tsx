...
import { getFoodItems, saveFoodItem } from '../../store/db';
...
export default function FoodCatalog() {
  ...
  function handleSoftDelete(item: FoodItem) {
    saveFoodItem({ ...item, status: 'deleted' });
    setRefreshKey((k) => k + 1);
    setDeleteConfirm(null);
  }

  function handleRestore(item: FoodItem) {
    saveFoodItem({ ...item, status: 'active' });
    setRefreshKey((k) => k + 1);
  }
                    ...
                    <td className="px-5 py-3.5">
                      <span
                        className={`font-medium text-gray-900 ${
                          item.status === 'deleted' ? 'line-through text-gray-400' : ''
                        }`}
                      >
                        {item.name}
                      </span>
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full ${
                          categoryBadge[item.category] ?? 'bg-gray-100 text-gray-600'
                        }`}
                      >
                        {item.category}
                      </span>
